...
const AdminDashboard = () => {
  ...
  const [notRegisteredUsers, setNotRegisteredUsers] = useState([]); // Estado para usuarios no registrados
  ...
  const filteredCampers = useMemo(() => {
    ...
    if (activeFilter === "notRegistred") {
      filtered = notRegisteredUsers; // Usamos los usuarios no registrados
    } else {
        ...
        <header className="backdrop-blur-xl m-2 sticky top-0 z-10 bg-transparent">
          <div className="flex flex-col md:flex-row items-start justify-between px-4 py-3 md:px-6">
            ...
            <div className="flex flex-wrap gap-2 md:gap-4 items-center">
              {["notRegistred", "all", "pending", "complete"].map((f) => (
                <Badge
                  key={f}
